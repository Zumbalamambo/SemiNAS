...
class NASEncoderLayer(nn.Module):
    def __init__(self, args, hidden_size, dropout):
        super().__init__()
        self.args = args
        self.hidden_size = hidden_size
        self.dropout = dropout
        self.ops = nn.ModuleList()
        for k, v in OPERATIONS_ENCODER.items():
            self.ops.append(v(hidden_size, dropout))

    def forward(self, x, arch, **kwargs):
        return self.ops[arch](x, **kwargs)
...
class NASDecoderLayer(nn.Module):
    def __init__(self, args, hidden_size, dropout):
        ...
        self.dropout = dropout
        self.ops = nn.ModuleList()
        for k, v in OPERATIONS_DECODER.items():
            self.ops.append(v(hidden_size, dropout))
    ...
    def clear_buffer(self, *args):
        for layer in self.ops:
            layer.clear_buffer(*args)

    def set_buffer(self, *args):
        for layer in self.ops:
            layer.set_buffer(*args)
...
class NASEncoder(nn.Module):
    def __init__(self, args, embed_tokens):
        ...
        self.layers.extend([
            NASEncoderLayer(args, self.hidden_size, self.dropout)
    ...
    def forward(self, src_tokens, arch, **kwargs):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, arch[i], encoder_padding_mask=encoder_padding_mask, **kwargs)
...
class NASDecoder(nn.Module):
    def __init__(self, args, padding_idx=0):
        super().__init__()
        self.args = args
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            NASDecoderLayer(args, self.hidden_size, self.dropout)
            for i in range(self.num_layers)
        ])
...
class NASNetwork(nn.Module):
    ...
    def build_encoder(self):
        return NASEncoder(self.args, self.encoder_embed_tokens)

    def build_decoder(self):
        return NASDecoder(self.args, padding_idx=self.padding_idx)

    def forward_encoder(self, src_tokens, arch, **kwargs):
        if isinstance(arch, str):
            arch = [int(i)-1 for i in arch.strip().split()]
        else:
            assert isinstance(arch, list)
            arch = [i-1 for i in arch]
        if len(arch) > self.enc_layers:
            arch = arch[:self.enc_layers]
        return self.encoder(src_tokens, arch, **kwargs)

    def forward_decoder(self, prev_output_mels, encoder_out, encoder_padding_mask, incremental_state=None, arch=None, **kwargs):
        if isinstance(arch, str):
            arch = [int(i)-1 for i in arch.strip().split()]
        else:
            assert isinstance(arch, list)
            arch = [i-1 for i in arch]
        if len(arch) > self.dec_layers:
            arch = arch[-self.dec_layers:]
        decoder_output, attn_logits = self.decoder(
            prev_output_mels, encoder_out, encoder_padding_mask, arch=arch, incremental_state=incremental_state, **kwargs)
        return decoder_output, attn_logits

    def forward(self, src_tokens, prev_output_mels, target_mels, arch, **kwargs):
        if isinstance(arch, str):
            arch = [int(i)-1 for i in arch.strip().split()]
        else:
            assert isinstance(arch, list)
            arch = [i-1 for i in arch]
        encoder_outputs = self.encoder(src_tokens, arch[:self.enc_layers], **kwargs)
        encoder_out = encoder_outputs['encoder_out']
        encoder_padding_mask = encoder_outputs['encoder_padding_mask']
        decoder_output, attn_logits = self.decoder(prev_output_mels, encoder_out, encoder_padding_mask, target_mels=target_mels, arch=arch[-self.dec_layers:], **kwargs)
        return decoder_output, attn_logits
