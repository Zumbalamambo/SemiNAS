...
class DecConvLayer(nn.Module):
    ...
    def forward(self, x, encoder_out=None, encoder_padding_mask=None, incremental_state=None, **kwargs):
        ...
        x, attn = self.attention(
            ...
            static_kv=True,
            enc_dec_attn_constraint_mask = utils.get_incremental_state(self, incremental_state, 'enc_dec_attn_constraint_mask')
            )
        x = F.dropout(x, self.dropout, training=self.training)
        x = residual + x
        attn_logits = attn[1]
...
class DecSALayer(nn.Module):
    def __init__(self, c, num_heads, dropout, attention_dropout=0.1, relu_dropout=0.1):
        ...
        self.ffn = TransformerFFNLayer(c, 4*c, padding='LEFT', kernel_size=9, dropout=relu_dropout)
...
OPERATIONS_ENCODER = {
    1: lambda c, dropout: EncConvLayer(c, 1, dropout),
    ...
    8: lambda c, dropout: EncSALayer(c, 2, dropout),
    9: lambda c, dropout: EncSALayer(c, 4, dropout),
    10: lambda c, dropout: EncSALayer(c, 8, dropout),
    11: lambda c, dropout: EncLSTMLayer(c, dropout),
}
...
OPERATIONS_DECODER = {
    ...
    8: lambda c, dropout: DecSALayer(c, 2, dropout),
